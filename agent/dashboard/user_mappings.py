"""Store-backed bidirectional GitHub ⇄ work-email user mapping.
...
keep an in-process cache of ``{login, email} -> record`` that async readers
refresh from the Store. The cache is best-effort: a cold cache falls back to
an async Store read where the call site allows it, and sync call sites degrade
to "unmapped" (the same conservative behavior as a missing dict entry).
"""
...
MappingSource = Literal["dashboard"]
...
async def upsert_mapping(
    *,
    github_login: str,
    work_email: str,
    source: MappingSource = "dashboard",
